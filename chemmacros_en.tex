...
\begin{options}
  ...
  \keyval{partial-format}{\LaTeX\ code}\Module{charges}\Default{\cs*{tiny}}
    ...
    section~\vref{sec:own-charge-macros}).
\end{options}
...
\begin{commands}
  \command{iupac}[\marg{IUPAC name}]
    Inside this command use \sym{\textbar} and \sym{-} to indicate a breaking
...
achieve (nearly) the same thing by using \sym{-} instead of \sym{\textbar},
and \cs*{textsuperscript} instead of \sym{\textasciicircum} without
\cs{iupac}.

There are some subtleties:
\begin{itemize}
  \item \sym{-} inserts a small space before the hyphen and removes a small
    space after it.  Also usually words with hyphens are only allowed to break
    at the hyphen.  Inside \cs{iupac} the hyphen will not prevent further
    hyphenation.  The amount of inserted space can be customized.
