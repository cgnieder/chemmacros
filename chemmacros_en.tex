...
\begin{itemize}
  ...
  \item A comprehensive set of macros! If there are any needs you might have
    with respect to typeseeting of chemistry which is not supported by this
  ...
  \item The commands shall not only make typesetting easier and faster but
    ...
    \code{\cs*{textit}\Marg{o}-dichlorobenzene}); the latter in my opinion
    also is more in the spirit of \LaTeXe.
  \item As much customizability as I could think of so every user can adapt
    the commands to his or her own wishes.
...
recommendations I would welcome an email\footnote{See
  section~\vref{sec:sugg-bug-reports}.}  very much!
...
structure\sinceversion{5.0}.  One the one hand this eases maintenance but it
...
\begin{enumerate}
  \item Internal modules which provide underlying functionality or basic
    functionality which is not of direct interest from a user perspective.
...
parts~\ref{part:preloaded-modules} and~\ref{part:additional-modules} of this
manual.
...
section~\ref{sec:all-module}). Own modules (see
section~\vref{sec:own-modules}) are \emph{not} loaded through this, though,
and still have to be loaded additionally.

In part~\ref{part:preloaded-modules} the preloaded modules are described,
first the user modules then the internal ones, in
part~\ref{part:additional-modules} the other available modules are described,
...
\chemformula\ provides means of typesetting chemical formul\ae\ and reactions.
You'll will see its macros \cs{ch} and \cs{chcpd} every now and then in this
...
\begin{itemize}
  ...
  \item Not all of the features are available per default any more, for some
    the corresponding module has to be loaded explicitly.  If suddenly some
    commands or environments seem to be undefined this may be the reason.
...
\begin{options}
  \keyval{compatibility}{num}\Default{\chemmacrosversion}
    Let's you specify the version of \chemmacros\ you want to use.  Any number
    smaller than $5.0$ will load v4.7. \ie, the last version before
    \chemmacros\ got its modular structure.  Not using the option will always
    load the newest version.  Please note that you only can specify the
    \emph{number} of the version.  For a version \enquote{5.2c} you can only
    set compatibility mode \enquote{5.2} but not specify the subversion.
\end{options}

In your document you can check for the compatibility mode:
\begin{commands}
...
\begin{commands}
  ...
  \command{Ka} \Ka, depends on language settings, see
    section~\ref{sec:lang-module}.  The translations can be adapted.
  \command{Kb} \Kb
  \command{Kw} \Kw
  \command{pKa}[\oarg{num}] \cs{pKa}: \pKa, \cs{pKa}\Oarg{1}: \pKa[1], depends
    on language settings, see  section~\ref{sec:lang-module}.  The translations
    can be adapted.
...
\begin{commands}
  ...
  \command{scrp} \scrp\ scriptstyle positive charge for usage in
    \pkg{chemfig}'s~\cite{pkg:chemfig} formulas.
  \command{scrm} \scrm\ scriptstyle negative charge for usage in
    \pkg{chemfig}'s formulas.
  \command{fscrp} \fscrp\ scriptstyle formal positive charge for usage in
    \pkg{chemfig}'s formulas.
  \command{fscrm} \fscrm\ scriptstyle formal negative charge for usage in
    \pkg{chemfig}'s formulas.
  \command{fsscrp} \fsscrp\ scriptscriptstyle formal positive charge for
    usage in \pkg{chemfig}'s formulas.
  \command{fsscrm} \fsscrm\ scriptscriptstyle formal negative charge for
    usage in \pkg{chemfig}'s formulas.
\end{commands}
...
section~\ref{sec:redox-module}) or with the \pkg{chemfig} package:
\begin{example}
  \chemsetup{
    charges/circled = all,
    redox/parse     = false,
...
macros like those described in section~\ref{sec:ion-charges}.
...
section~\ref{sec:part-charg-simil}. 
\begin{commands}
...
\begin{commands}
  \command{iupac}[\marg{IUPAC name}]
    Inside this command use \sym{\textbar} and \cs{-} to indicate a breaking
    point or a breaking dash.  Use \sym{\textasciicircum} as a shortcut for
    \cs*{textsuperscript}.
\end{commands}
...
\sym{\textasciicircum} without \cs{iupac}.

There are some subtleties: \cs{-} inserts a small space before the hyphen and
removes a small space after it.  The character \sym{\textbar} not only
prevents ligatures but also inserts a small space.
\begin{example}[side-by-side]
  \huge\iupac{2,4\-Di|chlor|pentan} \par
  2,4-Dichlorpentan
\end{example}

The spaces inserted by \cs{-} and \sym{\textbar} can be customized.
\begin{options}
...
\cs{E}, \cs{L}, \cs{R}, \cs{S}, \cs{trans} and so forth\footnote{Please read
  section~\ref{sec:one-letter-commands} before you consider using the
...
command (see section~\ref{sec:acid-base-module}).  For all other one-letter
...
read section~\ref{sec:greek-module} and also refer to \chemgreek's
documentation.
...
existing ones:
\begin{commands}
  \command{NewChemIUPACShorthand}[\meta{shorthand token}\meta{arg
    spec}\marg{code}]
    Defines a new \ac{iupac} shorthand.  This throws an error if
    \meta{shorthand token} is already defined.  The syntax is similar to
    \cs*{def}, \ie, you can define shorthands with arguments:

    \verbcode+\NewChemIUPACShorthand*#1{something with #1}+
  \command{RenewChemIUPACShorthand}[\meta{shorthand token}\meta{arg
    spec}\marg{code}]
    Redefines an existing \ac{iupac} shorthand.  This throws an error if
    \meta{shorthand token} is not defined, yet.  The syntax is the same as for
    \cs{NewChemIUPACShorthand}.
  \command{DeclareChemIUPACShorthand}[\meta{shorthand token}\meta{arg
    spec}\marg{code}]
    Defines a new \ac{iupac} shorthand or redefines an existing one.  The
    syntax is the same as for \cs{NewChemIUPACShorthand}.
  \command{ProvideChemIUPACShorthand}[\meta{shorthand token}\meta{arg
    spec}\marg{code}]
    Provides a new \ac{iupac} shorthand.  Does nothing if \meta{shorthand
      token} is already defined.  The syntax is the same as for
    \cs{NewChemIUPACShorthand}.
...
details are explained in section~\ref{sec:defin-own-part}.
...
See section~\ref{sec:lang-module} for predefined translations and general
...
(see section~\ref{sec:own-modules} for details).
...
\begin{commands}
  \expandable\command*{chemmacros_is_int:n}[\TF{} \marg{number}]
    Checks if \meta{number} is an integer or not.
  \expandable\command*{chemmacros_if_loaded:nn}[\TF{}
  \Marg{\meta{package}|\meta{class}}\marg{name}]
  ...
  \expandable\command*{chemmacros_if_package_loaded:n}[\TF{} \marg{name}]
    Checks if package \meta{name} has been loaded. Also works after begin document.
  \expandable\command*{chemmacros_if_class_loaded:n}[\TF{} \marg{name}]
    Checks if class \meta{name} has been loaded. Also works after begin document.
  ...
  \command*{chemmacros_bold:n}[ \marg{text}]
    ...
    \cs*{textbf} if in text mode or in \cs*{bm} if in math mode.
...
translation keys are listed in section~\ref{sec:avail-transl-keys}.
...
\begin{options}
  \keyval{list-name}{name of the list}\Module{reaction}\Default{List of
    ...
    dependent, see section~\ref{sec:lang-module}.
...
read section~\ref{sec:xfrac-module}.
...
see section~\ref{sec:lang-module} for details. If you want to manually change
...
me (see section~\ref{sec:submitting-module}) so they can be added to the
...
section~\ref{sec:submitting-module}) please follow the expl3 naming
