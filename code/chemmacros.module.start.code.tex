...
%  - chemmacros.sty, chemmacros4.sty, chemmacros.sty,
...
% --------------------------------------------------------------------------
% package information:
\tl_const:Nn \c_chemmacros_date_tl {2020/11/29}
\tl_const:Nn \c_chemmacros_version_major_number_tl {6}
\tl_const:Nn \c_chemmacros_version_minor_number_tl {}
...
\msg_new:nnn {chemmacros} {module-mechanism}
  {
    The~ module~ mechanism~ has~ changed~ with~ v5.12. \\
    All~ modules~ have~ been~ integrated~ into~ the~ main~ package~ and~ thus~
    are~ preloaded~ already.~ This~ corresponds~ to~ the~ earlier~ setting~
    `modules=all'
  }
