...
\chemsetup[reactants]{acronym-support=acro}
...
\begin{commands}
  ...
  This command is used to insert a predefined reactant with the \meta{ID} in the 
  text. Depending on the settings and the \option{reactant-output-style} option, 
  ...
  \code{*}, \code{+}, \code{l}, \code{s} or \code{plain} will be described later.
  ...
  data in the text. Format and order depend on the on the \option{solvent-output-style} option. \code{s} 
  and \code{l} suffixed variant exist and are discussed later.
\end{commands}
...
automatically adapts to the document language. Currently, the  English fallback, as well as 
the German translation are included in the package. If you write in a different language (or 
don't like the predefined text), you can use \cs{DeclareChemTranslation}\marg{key}\marg{language}
\marg{translation} (with \marg{key} = \code{solution}) as described in section~\vref{sec:lang-module}.
...
\begin{example}
  ...
  \reactant[equiv=2.0, amount=5, main=default]{dichloropentane}
\end{example}
...
\begin{commands}
  \command{reactant+}\oarg{data and units}\marg{ID}
  This command is used to insert a predefined reactant with the \meta{ID} in a section 
  heading or caption.
\end{commands}

If you also use the \pkg{hyperref} package in combination with \ac{pdf} bookmarks, you might 
want to use the optional \option{bookmark} option to supply an alternative text to \option{name}
to be displayed inside of the \ac{pdf} bookmarks. To later use such a predefined solvent or
reactant, use one of the following three commands, that are defined analogously to 
\pkg{chemnum}`s \cs{cmpdplain}.

\begin{commands}
  \command{reactantplain}\marg{ID}
  Outputs the value of \option{bookmark} instead of the \option{name} in section headings 
  and captions. 
  \command{submainreactantplain}\marg{mainID}\marg{subID}
  Outputs the value of \option{bookmark} instead of the \option{name} in section headings 
  and captions. Must be used if your \meta{ID} consists of a \meta{mainID} and a \meta{subID} 
  part. 
...
reactant/solvent should be used. Inspired by the \cs{acs} and \cs{acl} commands from 
the \pkg{acros} or the \pkg{glossaries-extra} package, the reactants module also offers 
the following \code{s} and \code{l} suffixed variants:
...
\begin{options}
  ...
  \meta{ID}s in combination with the starred variant \cs{printreactants*}.
\end{options}
