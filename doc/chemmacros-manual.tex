...
\DeclareChemReactant{dichlorpentan}{name={\iupac{2,4-di|chlor|pentan}}}

\begin{document}

\part{Preliminaries}
\section{Licence}\label{sec:licence-requ-readme}
\license
...
The \mcodule{reactants} module offers a simplified input syntax for chemical
...
The module requires and loads the packages \pkg{chemnum} and
\pkg{siunitx}. Depending on the selected options the packages \pkg{acro},
\pkg{glossaries-extra}, \pkg{hyperref}, and/or \pkg{xltabular} might be needed
for this module. Unlike other needed packages, these have to be loaded by the
user and will be explicitly mentioned in the corresponding sections of this
manual.
...
\begin{option}
  ...
  \keyval{short}{abbreviation}
    A short form of the name, used when the \module{reactants} module is used
    in combination with the \option{acronym-support} option.
  \keyval{bookmark}{replacement in \ac{pdf} bookmarks}
    Replaces \meta{name} in a \ac{pdf} bookmark.  This might be advisable when
    reactants are used in section titles and the \pkg{hyperref} is used as
    well.
\end{option}
...
with the help of \cs{DeclareChemReactant} will result in the same number as
...
keys listed in section~\vref{tab:reactant-data}.
...
solvent. This text automatically adapts to the document language.
...
\begin{example}
  ...
  \reactant[volume=5.00, amount=12.5, equiv=1.00, concentration=2.5, solvent=hexanes]{nBuLi}
