...
\pkg{xltabular}~\cite{pkg:xltabular} might be needed for this module. Unlike 
...
\begin{commands}
  ...
  \command{DeclareChemReactant}\Marg{\meta{main ID}.\meta{sub ID}}\marg{properties}
    Analogously to \pkg{chemnum}'s \cs{cmpd} the \cs{DeclareChemReactant} and
    \cs{reactant} commands accept a combined \meta{ID} consisting of a
    \meta{main ID} and \meta{sub ID} part.
\end{commands}
...
\begin{options}
  ...
  \keyval{short}{abbreviation}
    Optional property: a short form of the name, used when the \module{reactants} module is used
    in combination with the \option{acronym-support} option.
  \keyval{bookmark}{replacement in \ac{pdf} bookmarks}
    Optional property: replaces \meta{name} in a \ac{pdf} bookmark.  This might be advisable when
    reactants are used in section titles and the \pkg{hyperref} package is used as
    well.
\end{options}
...
\begin{commands}
  ...
  data in the text, depending on the \option{solvent-output-style} option. \code{s} 
...
\begin{options}
  \keybool{initiate}\Module{reactants}\Default{false}
    ...
    use of \module{reactant} being the default.   If you prefer to initiate a
...
readable, the \cs{reactant} and \cs{solvent} command offers an optional argument 
that can be used to easily input this data: \cs{reactant}\oarg{data and units}\marg{ID} 
\cs{solvent}\oarg{data and units}\marg{ID}.

\meta{list of data} accepts a comma separated list of key-value pairs with the available
...
Since numbers and their corresponding units are processed using \pkg{siunitx}, the usual 
\cs{sisetup} command can be used to alter for example the output decimal separator according 
to your needs. Be aware, though, that you must not use a comma as the the input decimal separator.
...
\cs{chemsetup} or in the optional argument of \cs{reactant}.
\begin{options}
...
\marg{translation} (with \marg{key} = \code{solution}) as described in section~
\vref{sec:lang-module}.
...
The names of the \option{reactant-output-style} and \option{olvent-output-style} choice options 
...
\begin{options}
  ...
  Be default, only \code{mass} and \code{volume} are assigned to the \code{main} category. Using 
  the \option{main} option,  \code{equiv} or \code{amount} can locally or globally assigned to 
  the \code{main} category, as well.
\end{options}

\begin{example}
  \chemsetup[reactants]{main=equiv}
  \reactant[equiv=2.0, amount=5]{dichloropentane}\par
  \reactant[equiv=2.0, amount=5, main=amount]{dichloropentane}\par
