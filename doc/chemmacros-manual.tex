...
\pkg{acro}~\cite{pkg:acro}, \pkg{glossaries-extra}~\cite{pkg:glossaries-extra}, \pkg{hyperref}~\cite{pkg:hyperref}, \pkg{longtable}~\cite{pkg:longtable} and/or 
...
\begin{options}
  \keyval{solution-name}{name}\Module{reactants}\Default{}
  \keybool{switch}\Module{reactants}\Default{false}
  \keybool{initiate}\Module{reactants}\Default{false}
...
\cs{reactant}\marg{ID}.  This command will return the name as well as the
corresponding number of this reactant. The starred variant
\cs{reactant}\sarg\marg{ID} will, with default settings, return just the name
of the substance without the number.  Setting \keyis{switch}{true} reverses
this behavior.

The use of \cs{cmpd}\marg{ID} with an \meta{ID} that was previously defined
with the help of the \cs{DeclareChemReactant} command will result in the same number as
using \cs{reactant}\marg{ID}.

\begin{example}
  \reactant{thf}
  \reactant*{thf}\par
  \chemsetup{reactants/switch = true}
  \reactant*{thf}
  \cmpd{thf}
\end{example}
...
readable, the \cs{reactant} command offers an optional argument that can be
used to easily input this data: \cs{reactant}\oarg{list of data}\marg{ID}.
\meta{list of data} accepts a comma separated list of key-value pairs with the
...
\begin{table}[htbp]
  ...
  \end{tabular}
\end{table}
\option{solution-name} here refers to the text that links concentration and
...
If you write in a different language (or don't like the predefined text), you can use \cs{DeclareChemTranslation}\marg{key}\marg{language}\marg{translation} (with \marg{key} = \code{solution}) as described in section~\vref{sec:lang-module}.
...
\begin{example}
  ...
  \reactant[volume=5.00, amount=12.5, equiv=1.00, concentration=2.5, solvent=hexane]{nBuLi}
  \reactant[mass=3.9, amount=15.6, equiv=1.3, purity=95]{Br2benzene}
